...
def run_session_verification() -> dict:
    """
    ┌─────────────────────────────────────────────────────────────┐
    │  SESSION 12.3 — VERIFICATION TEST                            │
    ├─────────────────────────────────────────────────────────────┤
    │  Checks 1-5 (Session 12.2, unchanged):                       │
    │    - chronicle_stream_events() yields at least 5 events      │
    │    - StreamStartEvent is the first event emitted             │
    │    - AgentHandoffEvent emitted for each of 5 agents          │
    │    - FinalAnswerEvent is the last event (final=True)         │
    │    - to_sse_frame() produces correct double-newline format   │
    │  Checks 6-8 (Session 12.3, new):                              │
    │    - job_store write_job/read_job/update_status round-trips  │
    │    - run_chronicle_analysis() is an async function            │
    │    - a full async job reaches status=completed with a        │
    │      non-empty final_result and active_node cleared          │
    └─────────────────────────────────────────────────────────────┘
    """
    import inspect
    from stream_schemas import (
    ...
    try:
        ...
        checks.append({"label": "chronicle_stream_events() runs", "passed": False, "note": str(e)})
        duration_ms = round((time.monotonic() - start) * 1000)
        return {"passed": False, "checks": checks, "summary": f"0/{8} checks passed in {duration_ms}ms", "duration_ms": duration_ms}
...
# ── CLI Entry Point (Session 12.3 — UPDATED) ─────────────────────

if __name__ == "__main__":
    print("\n╔══════════════════════════════════════════════════════╗")
    print("║  Chronicle — Session 12.3 Verification               ║")
    print("╚══════════════════════════════════════════════════════╝\n")
